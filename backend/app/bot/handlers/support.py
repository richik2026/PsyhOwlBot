...
def support_claim_keyboard(message_id: int):
    return InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="✏️ Взять в обработку", callback_data=f"support_claim:{message_id}")]])
...
async def send_support_prompt(message: Message, state: FSMContext):
    ...
    await message.answer("✏️ <b>Техническая поддержка Совёнка</b>\n\n📨 Напиши интересующий тебя вопрос и наша команда ответит тебе в течении пары мгновений!\n\n🗒 Отвечаем очень быстро с 05:00 — 00:00", parse_mode="HTML")
...
async def support_relay(message: Message, state: FSMContext, session: AsyncSession):
    text = ("❗️ <b>ВНИМАНИЕ, НОВОЕ ОБРАЩЕНИЕ!</b>\n\n"
...
async def ignore_after_support(message: Message):
    return
...
async def support_reply_handler(message: Message, session: AsyncSession):
    if message.chat.id != SUPPORT_GROUP_ID:
        return
    result = await session.execute(SupportMessage.__table__.select().where(SupportMessage.support_message_id == message.reply_to_message.message_id))
